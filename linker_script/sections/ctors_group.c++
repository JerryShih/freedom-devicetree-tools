...
{
  leading_commands.push_back(". = ALIGN(4);");
  ...
  Section finit_array(logical_memory, virtual_memory, logical_header);

  finit_array.output_name = "finit_array";

  finit_array.add_command("PROVIDE_HIDDEN (__finit_array_start = .);");
  finit_array.add_command("KEEP (*(SORT_BY_INIT_PRIORITY(.fini_array.*) SORT_BY_INIT_PRIORITY(.dtors.*)))");
  finit_array.add_command("KEEP (*(.fini_array EXCLUDE_FILE (*crtbegin.o *crtbegin?.o *crtend.o *crtend?.o ) .dtors))");
  finit_array.add_command("PROVIDE_HIDDEN (__finit_array_end = .);");

  sections.push_back(finit_array);
